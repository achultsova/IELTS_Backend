...
router.post('/login', body('email').isEmail().isLength({ max: 256 }).withMessage('Email length must be no more than 256 symbols'), body('password').isEmail().isLength({ max: 256 }).withMessage('Email length must be no more than 256 symbols'), userController.login);
